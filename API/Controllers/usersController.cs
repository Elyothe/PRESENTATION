...
namespace API.Controllers
{
    ...
    public class usersController : ControllerBase
    {
        ...
        {
            // Résoudre TokenService directement à partir du conteneur
            ...
            if (users!.u_Mail == null)
            {
                return Ok("Erreur : Aucun utilisateur à été trouvé !");
            }

            if (users!.idStatus == 1) // Exemple simplifié
            {
                var role = "Admin"; // Exemple de rôle
                var token = tokenService.GenerateToken(login.Username, role);
                return Ok(new { token });
            }
            else if(users!.idStatus == 0) 
            {
